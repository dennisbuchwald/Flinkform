# Flinkform Bridge Layer — Free/Pro Contract

This directory defines the **stable API** the free core exposes for the
**Flinkform Pro** add-on to dock onto. It is the heart of the Free/Pro split
...
add_filter( 'flinkform_pro_features', static function ( array $features ): array {
...
add_action( 'flinkform_register_modules', static function (): void {
    // new \FlinkformPro\Webhooks\…()->register();  etc.
} );
```
...
### 1. `flinkform_pro_features` (filter) — capability advertisement
The single source of truth for "is this Pro feature available?".
Resolved by `Flinkform\Bridge\Features`. Free core returns `[]` → all Pro
...
### 2. `flinkform_register_modules` (action) — module foothold
...
### 3. `flinkform_block_dirs` (filter) — block / field-type registration
...
- Applied in `Flinkform\Blocks\Registry::register_blocks()`.

### 4. `flinkform_spam_providers` (filter) — spam / CAPTCHA providers
...
- Consulted in `Flinkform\Spam\Guard::resolve_strategy()`.

### 5. `flinkform_submissions_table_actions` (action) — submissions filter-bar actions
...
- Fired in `Flinkform\Admin\SubmissionsListTable` (filter-bar render).
- Pro's request handler runs independently on `admin_init`, re-checking
  `Menu::CAPABILITY` + the `flinkform_export` nonce — the free core's `dispatch()`
  no longer routes an `export` action.

---

### 6. `flinkform.formContainer.inspectorPanels` (JS filter) — editor inspector panels
...
- **`flinkform_notification_routes` (filter)** — multiple notification routes
  (extra recipients, CRM). Deferred from M-a on purpose: the `Notifications\Mailer`
  is already cleanly factored with a working `flinkform_email_notification` filter;
