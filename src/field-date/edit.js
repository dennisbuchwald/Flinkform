...
export default function Edit( { attributes, setAttributes, context, clientId } ) {
	const { label, required, helpText, fieldName, minDate, maxDate } = attributes;
	const blockProps = useBlockProps( { className: 'flinkform-field flinkform-field--date' } );

	useEffect( () => {
		if ( ! fieldName ) {
			setAttributes( { fieldName: generateFieldName( 'date' ) } );
		}
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Field Settings', 'flinkform' ) }>
					...
					/>
					<TextControl
						label={ __( 'Earliest Date', 'flinkform' ) }
						type="date"
						value={ minDate }
					...
					/>
					<TextControl
						label={ __( 'Latest Date', 'flinkform' ) }
						type="date"
						value={ maxDate }
			...
			<div { ...blockProps }>
				<label className="flinkform-field__label">
					{ displayDefaultLabel( label, 'Date' ) }
