 ...
 * that intent flows in via the `perform/appearance` block context the
...
export default function FullWidthPanel( { attributes, setAttributes, context } ) {
	const columns = context?.[ 'perform/appearance' ]?.columns;
	if ( columns !== 2 ) {
		return null;
	}

	return (
		<PanelBody title={ __( 'Layout', 'perform-forms' ) } initialOpen={ false }>
			<ToggleControl
				label={ __( 'Full width', 'perform-forms' ) }
				help={ __( 'Make this field span both columns of the form.', 'perform-forms' ) }
